...
export class ProjectRenderer {
  ...
  constructor(gl: GL) {
    ...
    this.program5_calcVerVel = createProgramFromSources(
      gl,
      genericVertexShader,
      appendCommonGlsl(project_5_calc_v_ver_from_p_fragmentShader)
    );
  }
  ...
  private renderCalcVerticalVelocity(
    verticalVelocityIn: Texture,
    tempP: Texture,
    verticalVelocityOut: Texture
  ) {
    const gl = this.gl;
    const program = this.program5_calcVerVel;
    gl.useProgram(program);
    const vertexCount = prepareProgramToRenderOutput(
      gl,
      program,
    ...
    validateDefined({ u_vertical_velocity });
    gl.uniform1i(u_vertical_velocity, verticalVelocityIn.texture_id);
    ...
    gl.uniform2f(
      u_texture_size,
      verticalVelocityIn.width,
      verticalVelocityIn.height
    );
